  ...
  template: `
    <div style="display:flex;gap:8px;align-items:center">
      ...
      <mat-form-field appearance="fill"
        ><mat-label>Username</mat-label><input matInput [formControl]="q.controls.username"
      /></mat-form-field>
      <mat-form-field appearance="fill"
...
export class EveryoneProductsComponent implements OnInit {
  products: Product[] = [];
  category = '';
  q: any;
  constructor(
    ...
    this.q = this.fb.group({ username: [''], product: [''], date: [''] });
  ...
  reload() {
    this.products = this.ps.filter({
      username: this.q.value.username ?? undefined,
